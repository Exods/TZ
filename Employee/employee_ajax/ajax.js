<<<<<<< HEAD:results/employee_ajax/ajax.js
...
function validate() {
    ...
    let fields = $("[required]");
    for (i = 0; i < fields.length; i++) {
        if (fields[i].value.trim().length == ''&& $('.g-recaptcha-response').val() == "") {
            $(fields[i]).after('<p class="validation">Поле обязательное для заполнения!</p>');
            errorFields.push(fields[i].name);
            $("#blockCapch").css('border','5px solid red');
            errorFields.push(1);
...
>>>>>>> fc18b7c72f4ef8f0241f52a864f1fb964e8cc8ef:results/notebook_ajax/ajax.js
